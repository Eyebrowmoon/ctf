...
int very_very_hard_filter(char* buf)
{
    ...
    while(target = strchr(target + 1, '('))
    {
        ...
        if(!((*(target-2) == "if" && ( target - buf < 0x3 || (isspace(*(target-3)) || *(target-3) == ';'))) || 
              target- buf < 0x3 || 
             (*(target-3) == "for" &&  ( target - buf < 0x4 || (isspace(*(target-4)) || *(target-4) == ';'))) ||
              target - buf < 0x5 ||
             (*(target-5) == "while" &&  ( target - buf < 0x6 || (isspace(*(target-6)) || *(target-6) == ';')))))
            return 1;
    }
    return 0;
}
